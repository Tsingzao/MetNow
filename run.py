...
transform = transforms.Compose([transforms.ToTensor(), transforms.CenterCrop(256)])#, transforms.Normalize((0.5,), (0.5,))])
...
train_loader = DataLoader(trainset, batch_size=8, shuffle=True, num_workers=32)
...
model_name = UNet # SmaAt_UNet # SegFormer #
...
class Autoencoder(pl.LightningModule):
    def __init__(
        ...
        self.net = model_class(num_inchannels=5, num_classes=20)
    ...
    def _get_reconstruction_loss(self, batch):
        ...
        x, y = batch[:,:5], batch[:,5:]  # We do not need the labels
        y_hat = self.forward(x)
        loss = F.l1_loss(y, y_hat, reduction="none")
        loss = loss.mean(dim=[0,1,2,3])#.sum(dim=[1, 2, 3])
        return loss
...
class GenerateCallback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        if trainer.current_epoch % self.every_n_epochs == 0:
            ...
            input_imgs = self.input_imgs[:,:5].to(pl_module.device)
            with torch.no_grad():
                pl_module.eval()
